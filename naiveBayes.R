...
########### fit paramaters
  if(any(normalFeats)){
    for(iclass in 1:nClasses){
      ...
      mu <- colMeans(X[currentClassIndices,normalFeats], na.rm=TRUE)
      sigma <- apply(X[currentClassIndices,normalFeats], 2, fun <- function(x){sd(x, na.rm=TRUE)}) # column sd's
  ...
  if(any(mvmnFeats)){
    possibleFeatVals <- apply(X=X[,mvmnFeats], MARGIN=2, FUN=function(x){list(sort(unique(x)))}) # find the observed values for each feature
    for(iclass in 1:nClasses){
      currentClassIndices <- y==classLevels[iclass]
      ...
      for(ifeat in 1:length(mvmnFeatInds)){
        nFeatValObs <-featureValueObservations(observedFeatVals=X[currentClassIndices,mvmnFeatInds[ifeat]], possibleFeatVals=unlist(possibleFeatVals[[ifeat]])) # times that each value of feature was observed
...
featureValueObservations <- function(observedFeatVals, possibleFeatVals){
  counts <- sapply(possibleFeatVals, FUN=function(x, observedFeatVals){sum(x==observedFeatVals)}, observedFeatVals=observedFeatVals)
  counts
}
