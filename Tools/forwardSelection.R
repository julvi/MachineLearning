forwardSelection <- function(cost_function, X, y, cvK=10, minCostImprovement=0, nfeats=c(), stoppingCrit='nfeats', maxIts=Inf){
  #TODO: write CV function for forwardSelection to use if cvTools is not available
  # Description:
  ...
  reqres <- require(cvTools)
  if(!reqres){
    stop('forwardSelection: the package cvTools is required to run forwardSelection.')
  }
  CV <- cvFolds(length(y), K=cvK)
  ...
  while(keepGoing && counter<maxIts && length(unusedFeats)>0){
    ...
    for(ifeat in 1:length(unusedFeats)){
      ...
      cvCosts <- c()
      for(k in 1:cvK){
        cvCosts[k] <- cost_function(X[CV$which!=k,currentFeatset], y[CV$which!=k], X[CV$which==k,currentFeatset], y[CV$which==k])
      }
